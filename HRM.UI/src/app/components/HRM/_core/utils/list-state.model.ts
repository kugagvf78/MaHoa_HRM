...
export class ListStateModel {
	...
	private updateItem(_item: any) {
		if (!_item.id) {
			const c_index = _.findIndex(this.addedItems, function (o: any) { return o === _item._prevState; });
			if (c_index > -1) {
				this.addedItems[c_index] = _item;
				this.addedItems[c_index]._prevState = _item;
			}
		} else {
			const u_index = _.findIndex(this.updatedItems, function (o: any) { return o.id === _item.id; });
			if (u_index > -1) {
	...
	private deleteItem(_item: any) {
		if (_item.id) {
			const d_index = _.findIndex(this.deletedItems, function (o: any) { return o.id === _item.id; });
			const u_index = _.findIndex(this.updatedItems, function (o: any) { return o.id === _item.id; });
			if (d_index === -1) {
		...
		if (!_item.id) {
			const c_index = _.findIndex(this.addedItems, function (o: any) { return o === _item; });
			if (c_index > -1) {
