...
export class SubheaderService implements OnDestroy {
  ...
  setDefaultSubheader(): void {
    this.setSubheaderVersion(this.layout.getProp('subheader.layoutVersion') as string);
  }
  ...
  updateAfterRouteChanges(pathName: string): void {
  ...
  private parseUrlAndReturnPathname(href: string): string {
    ...
    url.href = href; // Sử dụng tham số href
    return url.pathname;
  }

  private getTitle(breadCrumbs: BreadcrumbItemModel[], pathname: string): string {
