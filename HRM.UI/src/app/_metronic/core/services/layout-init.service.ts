...
// Interface cho cấu hình layout (đã định nghĩa trong LayoutService)
...
export class LayoutInitService {
  ...
  private initAside() {
    ...
    console.log('Initializing Aside');
    if (this.layout.getProp('aside.self.display') !== true) {
      return;
    }
    ...
    const asideMenuSubmenuDropdownHoverTimeout = this.layout.getProp(
      'aside.menu.submenu.dropdown.hoverTimeout'
    );
    if (asideMenuSubmenuDropdownHoverTimeout) {
      this.layout.setHTMLAttribute(
        'aside_menu',
        'data-menu-dropdown-timeout',
        asideMenuSubmenuDropdownHoverTimeout
      );
    }
  }
