 ...
 */
	...
	public getUserRoles(): Observable<any> {
		...
		try {
			return of(roles ? JSON.parse(roles) : null); // Kiểm tra roles trước khi parse
		} catch (e) {
			return of(null); // Trả về giá trị mặc định trong trường hợp lỗi
		}
	}
	...
	public getMenuRoles(): Observable<any> {
		const roles = localStorage.getItem('menuRoles');
		try {
			return of(roles ? JSON.parse(roles) : null);
		} catch (e) {
			return of(null);
		}
	}
	...
	public getUrl(): Observable<any> {
		...
		try {
			return of(url ? JSON.parse(url) : null);
		} catch (e) {
			return of(null);
		}
	}
